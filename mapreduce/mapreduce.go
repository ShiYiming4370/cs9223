...
func ihash(s string) uint32 { //generate hash number for string s
	h := fnv.New32a()
	h.Write([]byte(s))
	return h.Sum32()
}
...
func DoMap(JobNumber int, fileName string,
	...
	size := fi.Size() //get file size
	fmt.Printf("DoMap: read split %s %d\n", name, size)
	b := make([]byte, size) //calculate total byte
	_, err = file.Read(b)
	if err != nil {
		log.Fatal("DoMap: ", err)
	}
	file.Close()
	res := Map(string(b)) // call Map Function written by ourselves
...
func DoReduce(job int, fileName string, nmap int,
	...
	for i := 0; i < nmap; i++ {
		...
		for {
			...
			_, ok := kvs[kv.Key]
			if !ok {
				kvs[kv.Key] = list.New() //add new key and its list
			}
			kvs[kv.Key].PushBack(kv.Value) //list of ""
		}
		file.Close()
	}
	var keys []string
